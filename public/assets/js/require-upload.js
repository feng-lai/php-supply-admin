                ...
                onUploadSuccess: function (up, ret, file) {
                    ...
                    if (button) {
                        ...
                        if (input_id) {
                            ...
                            //var url = JSON.stringify({url:data.url,name:data.name});
                            var url = Config.upload.fullmode ? Fast.api.cdnurl(data.url) : data.url;

                            urlArr.push(url);
                            inputObj.val(urlArr.join(",")).trigger("change").trigger("validate");
                        }
                        if (input_id == 'true-name') {

                            var inputObj = $("#" + input_id+'1');
                            var urlArr = inputObj.val()?inputObj.val():[]
                            urlArr.length?urlArr = JSON.parse(urlArr):''
                            var url = {url:data.url,name:data.name};

                            urlArr.push(url);
                            inputObj.val(JSON.stringify(urlArr)).trigger("change").trigger("validate");
                        }
                        //如果有回调函数
                    ...
                    $(element, Upload.config.container).each(function () {
                        ...
                        if (preview_id) {
                            ...
                            $(document.body).on("fa.preview.change", "#" + preview_id, function () {
                                ...
                                if(input_id == 'true-name'){
                                    let arr = JSON.parse($('#true-name1').val())
                                    let brr = []
                                    $("#" + preview_id + " [data-url]").each(function (i, j) {
                                        for(let i in arr){
                                            if(arr[i].url == $(this).data("url")){
                                                brr.push(arr[i])
                                            }
                                        }
                                    })
