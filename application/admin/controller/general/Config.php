...
class Config extends Backend
{
    ...
    protected $noNeedRight = ['check', 'rulelist', 'selectpage', 'get_fields_list','edit'];
    ...
    public function detail($name){
        $content = $this->model->where('name',$name)->value('value');
    ...
    public function serverset(){
        ...
        foreach ($this->model->all() as $k => $v) {
            ...
            if($value['name'] == 'rate_system' or $value['name'] == 'rate_custom' or $value['name'] == 'rate_exp_time'){
                $siteList[$v['group']]['list'][] = $value;
            }
    ...
    {
        $siteList = [];
        $groupList = ConfigModel::getAboutList();
        foreach ($groupList as $k => $v) {
            $siteList[$k]['name'] = $k;
            $siteList[$k]['title'] = $v;
            $siteList[$k]['list'] = [];
        ...
        foreach ($this->model->whereIn('name',['company_img','company_info','company_more'])->select() as $k => $v) {
            if (!isset($siteList[$v['group']])) {
                continue;
            }
            $value = $v->toArray();
    ...
    {
        $siteList = [];
        $groupList = ConfigModel::getContactList();
        foreach ($groupList as $k => $v) {
            $siteList[$k]['name'] = $k;
            $siteList[$k]['title'] = $v;
            $siteList[$k]['list'] = [];
        ...
        foreach ($this->model->where('group','contact')->select() as $k => $v) {
            if (!isset($siteList[$v['group']])) {
                continue;
            }
            $value = $v->toArray();
    ...
    {
        $siteList = [];
        $groupList = ConfigModel::getHomeList();
        foreach ($groupList as $k => $v) {
            $siteList[$k]['name'] = $k;
            $siteList[$k]['title'] = $v;
            $siteList[$k]['list'] = [];
        ...
        foreach ($this->model->where('group','home')->select() as $k => $v) {
            if (!isset($siteList[$v['group']])) {
                continue;
            }
            $value = $v->toArray();
